(function(){
  ...
  function boot(){
    ...
    if(w.__RHPRO_DESCONTOS_FIX_FINAL)return;
    w.__RHPRO_DESCONTOS_FIX_FINAL=true;
    w.__RHPRO_DESCONTO_EDIT_INDEX=-1;

    function ajustarStatusDesconto(){
      const sel=d.getElementById('descStatus'); if(!sel)return;
      const desejados=['Ativo','Quitado','Cancelado'];
      const atual=String(sel.value||'').trim();
      const ok=[...sel.options].length===3 && [...sel.options].every((o,i)=>o.textContent.trim()===desejados[i]);
      if(!ok){
        sel.innerHTML='';
        desejados.forEach(v=>{const o=d.createElement('option');o.value=v;o.textContent=v;sel.appendChild(o);});
        sel.value=desejados.includes(atual)?atual:'Ativo';
      }
    }
    ...
    w.renderDescontosRows=function(){
      ...
      rs.forEach(r=>{
        const i=all.indexOf(r), status=String(r[5]||'').trim();
        const key=status.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\s+/g,'-');
        const palette={ativo:'background:#183b63;color:#fff;',quitado:'background:#c62828;color:#fff;',cancelado:'background:#6b7280;color:#fff;'};
        h+='<tr><td>'+w.esc(r[0]||'')+'</td><td>'+w.esc(r[1]||'')+'</td><td>'+w.esc(r[2]||'')+'</td><td>'+w.esc(r[3]||'')+'</td><td>'+w.esc(r[4]||'')+'</td><td><span class="status-desconto '+w.esc(key)+'" style="display:inline-block;padding:7px 14px;border-radius:999px;font-weight:700;min-width:76px;text-align:center;white-space:nowrap;'+(palette[key]||'background:#64748b;color:#fff;')+'">'+w.esc(status||'—')+'</span></td><td>'+w.esc(r[6]||'')+'</td><td><button class="mini" type="button" data-desc-edit="'+i+'">✏️ Editar</button> <button class="mini" type="button" data-desc-del="'+i+'">Excluir</button></td></tr>';
      });
      h+='</tbody></table>';
    ...
    w.editarDesconto=function(i){
      ...
      set('descEmployee',r[1]);set('descType',r[2]);set('descComp',r[3]);set('descValue',r[4]);set('descNote',r[6]);
      ajustarStatusDesconto();set('descStatus',statusValido(r[5]));
    ...
    w.saveDesconto=function(){
      ...
      const msg=d.getElementById('descMsg');if(!r[1]){if(msg)msg.textContent='Selecione o colaborador.';return;}
    ...
    if(main&&!main.__descFixBoundFinal){
      main.__descFixBoundFinal=true;
    ...
    if(typeof w.showSection==='function'&&!w.__RHPRO_DESC_SHOW_FINAL){
      const original=w.showSection;
      w.showSection=function(name){
        const result=original.apply(this,arguments);
        if(String(name||'').trim()==='Descontos')ajustarStatusDesconto();
        return result;
      };
      w.__RHPRO_DESC_SHOW_FINAL=true;
    }

    if(String(d.querySelector('h1')?.textContent||'').trim()==='Descontos')ajustarStatusDesconto();
  }
