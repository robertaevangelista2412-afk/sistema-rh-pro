(function(){
  ...
  function start(){
    ...
    function form(idx){
      ...
      vals.forEach((c,j)=>{if(j===0){let z=date(old[j]);h+='<label>'+c+'<input id="af-f'+j+'" type="date" value="'+(z?z.getFullYear()+'-'+pad(z.getMonth()+1)+'-'+pad(z.getDate()):'')+'"></label>'}else if(j===1)h+='<label>'+c+'<input id="af-f1" inputmode="numeric" placeholder="9 ou 930" value="'+esc(time(old[j]))+'"><small>9 → 09:00H | 930 → 09:30H | 14 → 14:00H</small></label>';else if(j===6)h+='<label>'+c+'<select id="af-f6">'+['Agendado','Confirmado','Em andamento','Concluído','Cancelado'].map(o=>'<option '+((old[j]||'Agendado')===o?'selected':'')+'>'+o+'</option>').join('')+'</select></label>';else h+='<label>'+c+'<input id="af-f'+j+'" value="'+esc(old[j]||'')+'"></label>';});
    ...
    function draw(){
      ...
      body.onclick=e=>{let b=e.target.closest('button');if(!b)return;if(b.dataset.e!==undefined)form(+b.dataset.e);if(b.dataset.d!==undefined&&confirm('Excluir este compromisso?')){w.data['Agenda RH'].splice(+b.dataset.d,1);if(typeof w.save==='function')w.save();draw()}};
    }
    const style=d.createElement('style');style.id='agenda-fix-style';style.textContent='.af-bar{display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;margin-bottom:14px}.af-bar strong{font-size:22px;color:#123b67;min-width:220px;text-align:center}.af-info{font-size:13px;color:#6b7280;margin-bottom:12px}.af-empty{padding:30px;text-align:center;color:#6b7280}label small{display:block;color:#6b7280;font-weight:400;margin-top:5px;font-size:11px}@media(max-width:700px){.af-bar strong{order:-1;flex-basis:100%}.af-bar button{flex:1} }';if(!d.getElementById('agenda-fix-style'))d.head.appendChild(style);
    ...
    const oldShow=w.showSection;w.showSection=function(n){const r=oldShow.apply(this,arguments);if(n==='Agenda RH')setTimeout(draw,10);return r};
    new MutationObserver(watch).observe(d.body,{childList:true,subtree:true});setInterval(watch,300);watch();
  }
