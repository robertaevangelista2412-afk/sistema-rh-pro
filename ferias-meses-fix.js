...
// ALTERAÇÃO EXCLUSIVA: filtro visual por mês. NÃO altera, exclui ou recria os dados salvos.
(function(){
  ...
  function lerMes(d,key){
    try{
      const s=storage(d); const n=Number(s?.getItem(key));
  ...
  function mesDaLinha(tr,tipo){
    ...
    const t=tr.closest('table'); const idx=colunaData(t,tipo);
  ...
  function selecionar(tipo,n){
    ...
    const box=d.getElementById(id); if(box)pintar(box,n);
    filtrar(d,tipo,n);
  }
    ...
    if(!box){
      ...
      meses.forEach((nome,i)=>{
        ...
        b.addEventListener('click',function(ev){
          ev.preventDefault(); ev.stopImmediatePropagation(); selecionar(tipo,i); return false;
        },true);
        box.appendChild(b);
      });
      host.insertBefore(box,host.firstChild);
    }
    const n=lerMes(d,key); box.dataset.rhMesSelecionado=String(n); pintar(box,n); filtrar(d,tipo,n);
  }
  ...
  function boot(){
    ...
    if(ultimoDoc!==d){
      ...
      observer=new MutationObserver(function(){
        ...
        observer._timer=setTimeout(function(){criarAbas(d,'ferias');criarAbas(d,'docs');},60);
      });
  ...
  setInterval(function(){try{const d=frame.contentDocument;if(!d)return;const bf=d.getElementById('feriasMesesFix'),bd=d.getElementById('ferDocMesesFix');const mf=lerMes(d,KEY_F),md=lerMes(d,KEY_D);if(bf)pintar(bf,mf);if(bd)pintar(bd,md);filtrar(d,'ferias',mf);filtrar(d,'docs',md);}catch(e){}},500);
})();
